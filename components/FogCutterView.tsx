    ...
    <div className="px-n-6 pt-n-4 h-full flex flex-col pb-20">
      ...
      <div className="space-y-n-4">
        <textarea
          ...
          className="w-full h-32 bg-transparent n-border rounded-n-sm p-n-4 font-body text-sm text-n-white transition-colors duration-n-fast placeholder:opacity-25 placeholder:font-body resize-none"
        />

        <button
          onClick={handleCut}
          disabled={loading || !input.trim()}
          className={`w-full n-border rounded-n-sm p-n-4 ndot text-xl transition-all duration-n-med ease-n-ease ${
            loading
              ? 'opacity-40 animate-glyph-pulse'
              : 'n-press n-border-hover hover:bg-white/5'
      ...
      {results.length > 0 && !loading && (
        <div className="mt-n-8 space-y-n-3">
          <p className="n-label">Generated subtasks</p>
          {results.map((r, i) => (
            <div
              key={i}
              className="n-border rounded-n-sm p-n-4 bg-n-surface ndot text-base animate-fade-in-up anim-hidden"
          ...
          <button
            ...
            className="n-press w-full bg-n-white text-n-black rounded-n-sm p-n-4 ndot text-xl mt-n-4 transition-all duration-n-fast ease-n-ease hover:bg-white/90 active:bg-white/80"
          >
            ADD ALL TO TASKS
          </button>
        </div>
